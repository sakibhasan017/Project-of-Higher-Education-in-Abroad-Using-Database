...
<button class="insert-info-button" onclick="location.href='insertInfo.php'">Insert Information</button>
...
if ($stmt) {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td><a href='uniDetailsAdmin.php?name=" . urlencode($row["Ins_Name"]) . "'>" . $row["Ins_Name"] . "</a></td>";
            echo "<td>$" . number_format($row["Tuition_Fee"], 2) . "</td>";
            echo "<td>$" . number_format($row["Hall_Cost"], 2) . "</td>";
            echo "<td>" . $row["App_Last_Date"] . "</td>";
            echo "<td>" . $row["Location"] . "</td>";
            echo "<td><a href='deleteInstitute.php?Ins_ID={$row['Ins_ID']}' onclick='return confirm(\"Are you sure you want to delete this hall?\");'>Delete</a></td>";
            echo "</tr>";
        }
    } else {
